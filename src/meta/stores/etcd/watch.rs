...
impl WatchConfig {
    ...
    pub fn from_env_or_default() -> Self {
        Self {
            enabled: std::env::var("BREWFS_WATCH_ENABLED")
                .map(|v| v.to_lowercase() == "true")
                .unwrap_or(false),
            prefixes: std::env::var("BREWFS_WATCH_PREFIXES")
                .map(|v| {
                    v.split(',')
                ...
                .unwrap_or_default(),
            event_buffer_size: std::env::var("BREWFS_WATCH_BUFFER_SIZE")
                .ok()
                .and_then(|v| v.parse().ok())
                .unwrap_or(1000),
            debug: std::env::var("BREWFS_WATCH_DEBUG")
                .map(|v| v.to_lowercase() == "true")
                .unwrap_or(false),
        }
    }
...
mod tests {
    ...
    fn test_watch_config_from_env() {
        unsafe {
            std::env::set_var("BREWFS_WATCH_ENABLED", "true");
            std::env::set_var("BREWFS_WATCH_PREFIXES", "f:, r:");
        }
        let config = WatchConfig::from_env_or_default();
        assert!(config.enabled);
        assert_eq!(config.prefixes, vec!["f:", "r:"]);
        unsafe {
            std::env::remove_var("BREWFS_WATCH_ENABLED");
            std::env::remove_var("BREWFS_WATCH_PREFIXES");
        }
    }

...
//! Set BREWFS_TEST_S3_BUCKET environment variable to run these tests.
...
mod tests {
    ...
    async fn get_test_backend() -> Option<S3Backend> {
        let bucket = std::env::var("BREWFS_TEST_S3_BUCKET").ok()?;
    ...
    async fn test_s3_basic_operations() -> Result<(), Box<dyn Error>> {
        let backend = get_test_backend().expect("Set BREWFS_TEST_S3_BUCKET env var");

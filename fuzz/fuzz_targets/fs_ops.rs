...
fn compare_outcome<T, U>(op: &str, slayer: io::Result<T>, model: io::Result<U>) -> Option<(T, U)> {
    ...
    match (slayer, model) {
        ...
        (Ok(_), Err(e)) => panic!("{op}: brewfs succeeded but model failed: {e:?}"),
        (Err(e), Ok(_)) => panic!("{op}: brewfs failed but model succeeded: {e:?}"),
    }
}
    ...
    match (slayer_meta, model_meta) {
        (Ok(sm), Ok(mm)) => {
            ...
            if sm.kind == VfsFileType::File {
                ...
                let got_target = vfs.readlink(&sp).await.expect("brewfs readlink failed");
        ...
        (Err(_), Err(_)) => {}
        (Ok(_), Err(e)) => panic!("verify: existence mismatch, brewfs has {sp}, model err={e:?}"),
        (Err(e), Ok(_)) => panic!("verify: existence mismatch, model has {sp}, brewfs err={e:?}"),
    }
}
...
struct ModelDirFd {
    path: PathBuf,
    offset: u64,
    // Snapshot taken at open-time to mirror BrewFS DirHandle semantics.
    entries: Vec<String>,
}
...
fn fuzz_clients_from_env() -> usize {
    std::env::var("BREWFS_FUZZ_CLIENTS")
        .ok()
...
fn fuzz_meta_url_from_env() -> String {
    std::env::var("BREWFS_FUZZ_META_URL")
        .ok()
...
fn fuzz_etcd_urls_from_env() -> Vec<String> {
    std::env::var("BREWFS_FUZZ_ETCD_URLS")
        .or_else(|_| std::env::var("BREWFS_FUZZ_META_ETCD_URLS"))
        .ok()
        .map(|raw| {
            raw.split(',')
...
fn fuzz_meta_config_from_env() -> FuzzMetaConfig {
    let backend = std::env::var("BREWFS_FUZZ_META_BACKEND")
        .ok()
...
fn fuzz_concurrent_from_env() -> bool {
    std::env::var("BREWFS_FUZZ_CONCURRENT")
        .ok()
        .map(|v| {
            let v = v.trim();
...
fn fuzz_concurrent_batch_from_env() -> usize {
    std::env::var("BREWFS_FUZZ_CONCURRENT_BATCH")
        .ok()
        .and_then(|v| v.parse::<usize>().ok())
        .map(|v| v.clamp(1, 64))
        .unwrap_or(8)

...
| Data Path | **Reuse existing** `read_slices` / `write_slice` / `truncate_file` signatures (confirm semantics) | Ensure slice-level IO operations meet parity requirements | Document mapping between Go `Read/Write/Truncate` and current methods before considering renames. See `doc/meta-api/api-mapping.md`. |

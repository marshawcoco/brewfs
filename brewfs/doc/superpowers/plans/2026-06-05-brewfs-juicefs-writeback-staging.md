...
- Modify only if needed: `/mnt/slayerfs/brewfs/brewfs/docker/compose-xfstests/run_perf_in_container.sh`
  - Include new `.stats` metrics in reports if not already captured.
- Reference only: `/mnt/slayerfs/brewfs/brewfs/juicefs/pkg/chunk/cached_store.go`
- Reference only: `/mnt/slayerfs/brewfs/brewfs/juicefs/pkg/vfs/writer.go`
- Reference only: `/mnt/slayerfs/brewfs/brewfs/juicefs/pkg/chunk/disk_cache.go`
...
python3 - <<'PY'
import json, pathlib
root = max(pathlib.Path("brewfs/docker/compose-xfstests/artifacts").glob("perf-run-*"), key=lambda p: p.stat().st_mtime)
...
**Files:**
- Modify: `/mnt/slayerfs/brewfs/brewfs/src/vfs/io/writer.rs`
- Test: `/mnt/slayerfs/brewfs/brewfs/src/vfs/io/writer.rs`

- [ ] **Step 1: Write the failing unit test**

Add this test in `mod tests` in `/mnt/slayerfs/brewfs/brewfs/src/vfs/io/writer.rs`:
...
In `/mnt/slayerfs/brewfs/brewfs/src/vfs/io/writer.rs`, add to `DataWriter`:
...
cargo fmt --all -- --check
cargo clippy --workspace --all-targets -- -D warnings
bash brewfs/docker/compose-xfstests/run_redis_perf.sh --writeback-throughput-profile
```
...
```bash
cd /mnt/slayerfs/brewfs
git add brewfs/src/vfs/config.rs brewfs/src/vfs/io/writer.rs brewfs/src/vfs/stats.rs brewfs/src/vfs/fs/mod.rs

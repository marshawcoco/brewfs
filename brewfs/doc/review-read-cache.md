...
本次审查范围覆盖 `brewfs/src/vfs/io/reader.rs`、`brewfs/src/vfs/cache/*`、`brewfs/src/chunk/cache.rs`、`brewfs/src/chunk/page_cache.rs`、`brewfs/src/chunk/slice.rs`，并只读对照了 `brewfs/juicefs/pkg/vfs/reader.go` 与 `brewfs/juicefs/pkg/chunk/cached_store.go`。
...
- 位置：`brewfs/src/vfs/io/reader.rs::DataReader::submit_prefetch` 约 138-166 行；`brewfs/src/vfs/cache/prefetch.rs::GlobalPrefetcher::worker_loop` 约 135-199 行；`brewfs/src/chunk/store.rs::prefetch_full_block_background` 约 521-583 行。
...
- 位置：`brewfs/src/chunk/cache.rs::ChunksCache::get` 约 1665-1710 行；`brewfs/src/chunk/store.rs::ObjectStoreMetrics` 约 95-222 行；`brewfs/src/vfs/stats.rs` 约 80-89、553-574、907-950 行。
...
- 位置：`brewfs/src/chunk/store.rs::read_range` 约 697-812 行，尤其 806-810 行；`prefetch_full_block_background` 约 521-583 行。
...
- 位置：`brewfs/src/vfs/cache/config.rs::CacheConfig::default` 约 69-95 行默认 `compression=Lz4`；`brewfs/src/chunk/store.rs::read_range` 约 697 行要求 `Compression::None` 才走 range；约 821-876 行 full GET + decompression + cache insert。
...
- 位置：`brewfs/src/chunk/reader.rs::DataFetcher::read_at` 约 139-204 行；`read_at_into` 约 228-285 行。
...
- 位置：`brewfs/src/meta/client/mod.rs::MetaLayer for MetaClient::get_slices` 约 2317-2348 行；Redis backend `get_slices` 约 2980-2996 行；database backend 约 2759-2770 行；etcd backend 约 2533-2542 行；`DataFetcher::read_at` 约 120-133 行倒序 cut。
...
- 位置：`brewfs/src/vfs/io/reader.rs::FileReader` 字段 `chunk_slices` 约 492-495 行；`read_chunk_span` 约 858-879 行；`invalidate` 约 981-994 行；MetaClient inode cache slice map 约 `brewfs/src/meta/client/cache.rs` 342-400 行。
...
- 位置：`brewfs/src/chunk/store.rs::read_range` 约 871-876 行；`brewfs/src/chunk/cache.rs::insert_opportunistic` 约 1753-1818 行；`insert_hot` 约 1746-1751 行；`DiskStorage::store_with_permit` 约 374-436 行。
...
- 位置：`brewfs/src/chunk/page_cache.rs::ReadPageCache::new` 约 49-55 行；`ObjectBlockStore::new*` 约 376、446 行。
...
- 位置：`brewfs/src/fuse/mod.rs::fuse_open_reply_flags` 约 62-79 行；`brewfs/src/fuse/mount.rs::default_mount_options` 约 27-45 行。
...
- 位置：`brewfs/src/vfs/cache/read_cache.rs` 约 1-28 行注释 “currently bypassed by ObjectBlockStore”；`brewfs/src/vfs/cache/mod.rs` 约 1-7 行。
...
- 位置：`brewfs/src/chunk/cache.rs::DiskStorage::load` 约 510-566 行；`evict_lru` 约 455-508 行；`store_with_permit` 约 390-436 行。

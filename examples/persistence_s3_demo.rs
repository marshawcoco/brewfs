...
struct Args {
    ...
    #[arg(long, default_value = "./brewfs-meta")]
    meta_dir: PathBuf,
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    #[cfg(target_os = "linux")]
    {
        ...
        println!("=== BrewFS Persistence + S3 Demo ===");
        ...
        let meta_work_dir = meta_dir.join(".brewfs");
        std::fs::create_dir_all(&meta_work_dir)?;

        let target_config_path = meta_work_dir.join("brewfs.yml");
        ...
        let config = brewfs::Config::from_file(&target_config_path)
        ...
        println!(
            "BrewFS with S3 backend successfully mounted at: {}",
            mount_point.display()
        );

...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    #[cfg(target_os = "linux")]
    {
        ...
        println!("=== BrewFS Persistence Demo ===");
        ...
        let meta_config_dir = backend_storage.join(".brewfs");

        // Process config and decide whether we must write a backend_storage-local copy.
        // For sqlite we must rewrite the URL to point into backend_storage/.brewfs/metadata.db
        ...
        let config = if is_sqlite {
            ...
            let target_config_path = meta_config_dir.join("brewfs.yml");
            std::fs::write(&target_config_path, processed_config)?;

            brewfs::Config::from_file(&target_config_path)
                .map_err(|e| format!("Failed to load config file: {}", e))?
        } else {
            // Non-sqlite: load config directly from the user-provided file path
            brewfs::Config::from_file(&config_file)
                .map_err(|e| format!("Failed to load config file: {}", e))?
        };
        ...
        let block_store_for_gc: Arc<dyn brewfs::chunk::BlockStore + Send + Sync> =
        ...
        let gc_handle = tokio::spawn({
            ...
            async move {
                use brewfs::daemon::worker::start_gc;
                use std::sync::Arc;

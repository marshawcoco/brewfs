...
COMPOSE_FILE="$SCRIPT_DIR/docker-compose.ltp-etcd.yml"
...
description:
  - run LTP filesystem tests inside docker container against brewfs with etcd metadata backend
...
ts="$(date +%s)-$RANDOM"
PROJECT_NAME="brewfs-ltp-etcd-${ts}"
...
info "build etcd-maintenance image"
docker compose "${COMPOSE_ARGS[@]}" build etcd-maintenance
...
info "start dependency services: etcd + rustfs"
docker compose "${COMPOSE_ARGS[@]}" up -d etcd rustfs

info "start etcd maintenance"
docker compose "${COMPOSE_ARGS[@]}" up -d etcd-maintenance

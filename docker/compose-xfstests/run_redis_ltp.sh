...
COMPOSE_FILE="$SCRIPT_DIR/docker-compose.ltp-redis.yml"
...
description:
  - run LTP filesystem tests inside docker container against brewfs with redis metadata backend
...
ts="$(date +%s)-$RANDOM"
PROJECT_NAME="brewfs-ltp-redis-${ts}"

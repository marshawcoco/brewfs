...
BREWFS_DIR="$(realpath "$SCRIPT_DIR/../..")"
PROJECT_DIR="$(realpath "$BREWFS_DIR/..")"
...
REPORT_SCRIPT="$BREWFS_DIR/docker/xfstests_report.sh"
EXCLUDE_FILE="$BREWFS_DIR/tests/scripts/xfstests_slayer.exclude"
...
case "$BACKEND" in
    sqlite)
        TEST_NAME="test_brewfs_kvm_xfstests_sqlite"
        REQUIRED_SERVICES=()
        ;;
    redis)
        TEST_NAME="test_brewfs_kvm_xfstests_redis"
        REQUIRED_SERVICES=(redis)
        ;;
    etcd)
        TEST_NAME="test_brewfs_kvm_xfstests_etcd"
...
info "=== BrewFS xfstests 本地运行: backend=$BACKEND ==="
...
ARTIFACT_ROOT="${ARTIFACT_ROOT_VALUE:-/tmp/brewfs-kvm-xfstests/local/$BACKEND}"
mkdir -p "$ARTIFACT_ROOT"

info "运行 ignored xfstests 测试: $TEST_NAME"
cd "$PROJECT_DIR"

ENV_ARGS=("BREWFS_XFSTESTS_HOST_ARTIFACT_ROOT=$ARTIFACT_ROOT")
if [[ -n "$XFSTESTS_TIMEOUT_SECS_VALUE" ]]; then
    ENV_ARGS+=("BREWFS_XFSTESTS_TIMEOUT_SECS=$XFSTESTS_TIMEOUT_SECS_VALUE")
fi
if [[ -n "$XFSTESTS_FORCE_RECLONE_VALUE" ]]; then
    ENV_ARGS+=("BREWFS_XFSTESTS_FORCE_RECLONE=$XFSTESTS_FORCE_RECLONE_VALUE")
fi

env "${ENV_ARGS[@]}" \
    cargo test -p brewfs --test test_brewfs_kvm_integration \

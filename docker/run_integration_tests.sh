#!/usr/bin/env bash
# 用途:
#   在本地运行 brewfs 集成测试，流程与 CI (brewfs-tests.yml) 保持一致。
#   支持以下测试阶段：
...
# --------------------------------------------------------------------------- #
info "=== 运行集成测试: test_brewfs_qlean_multinode_smoke ==="
cd "$PROJECT_DIR"
cargo test -p brewfs --test test_brewfs_qlean_multinode_smoke -- --ignored
ok "集成测试通过"
...
if [[ "$RUN_FUZZ" == true ]]; then
    ...
    if [[ "$FUZZ_STATUS" -eq 71 ]]; then
        ...
        err "产出物位于: $BREWFS_DIR/fuzz/artifacts/fs_ops/"
        exit "$FUZZ_STATUS"
    else
        ok "Fuzz 探索完成"
    fi

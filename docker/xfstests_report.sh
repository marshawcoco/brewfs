...
示例:
  $(basename "$0") /tmp/brewfs-kvm-xfstests/local/redis
  $(basename "$0") /tmp/brewfs-kvm-xfstests/local/redis/1712345678-123-pid9999
EOF
    exit 0
}
...

{
    ...
    echo
    echo "key_files:"
    for f in "brewfs.log" "xfstests-script.log" "local.config" "results/check.out" "results/check.log"; do
        if [[ -e "$run_dir/$f" ]]; then
            echo "  - $f"
        else
            echo "  - $f (missing)"

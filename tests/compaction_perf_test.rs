...
//!
//! Run with: cargo test -p brewfs --test compaction_perf_test -- --nocapture
//!
//! Environment variables:
//! - BREWFS_TEST_BACKEND: "sqlite" (default), "etcd", or "redis"
//! - BREWFS_TEST_ETCD_URL: etcd endpoints, default "http://localhost:2379"
//! - BREWFS_TEST_REDIS_URL: redis URL, default "redis://localhost:6379"
// WIP！！！

#[cfg(test)]
mod common;

use brewfs::meta::SLICE_ID_KEY;
use brewfs::{
...
fn get_backend_type() -> String {
    std::env::var("BREWFS_TEST_BACKEND").unwrap_or_else(|_| "sqlite".to_string())
}
...
async fn setup_test_fs() -> (
    ...
    match backend.as_str() {
        "etcd" => {
            let etcd_url = std::env::var("BREWFS_TEST_ETCD_URL")
        ...
        "redis" => {
            let redis_url = std::env::var("BREWFS_TEST_REDIS_URL")
        ...
        _ => {
            ...
            let config = Config {
                database: brewfs::DatabaseConfig {

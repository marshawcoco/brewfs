...
if [[ -z "$persistence_bin" ]]; then
    echo "Cannot find brewfs persistence_s3_demo binary."
    echo "Please run: cargo build -p brewfs --example persistence_s3_demo --release"
    exit 1
fi
...
sudo cat >/usr/sbin/mount.fuse.brewfs  <<EOF
#!/bin/bash
set -euo pipefail

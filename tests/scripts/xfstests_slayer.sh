...
brewfs_rust_log="${brewfs_rust_log:-brewfs=info,asyncfuse::raw::logfs=debug}"
brewfs_fuse_op_log="${brewfs_fuse_op_log:-1}"

if [[ ! -f "$persistence_bin" ]]; then
    echo "Cannot find brewfs persistence_demo binary."
    echo "Please run: cargo build -p brewfs --example persistence_demo --release"
    exit 1
fi
...
#export SCRATCH_DEV=brewfs
#export SCRATCH_MNT=/tmp/test2/merged
export FSTYP=fuse
export FUSE_SUBTYP=.brewfs

#Deleting the following command will result in an error: TEST_DEV=brewfs is mounted but not a type fuse filesystem.
export DF_PROG="df -T -P -a"
EOF

# create fuse mount script for brewfs.
sudo tee /usr/sbin/mount.fuse.brewfs >/dev/null <<EOF
#!/bin/bash
set -euo pipefail
...
MOUNT_DIR="$mount_dir"
BREWFS_RUST_LOG="$brewfs_rust_log"
BREWFS_FUSE_OP_LOG="$brewfs_fuse_op_log"

if [[ "\$BREWFS_FUSE_OP_LOG" == "1" ]]; then
  export RUST_LOG="\$BREWFS_RUST_LOG"
fi
...
sleep 1
EOF
sudo chmod +x /usr/sbin/mount.fuse.brewfs
